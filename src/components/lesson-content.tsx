...
function CodeBlock({ language, code, filename }: { language: string; code: string; filename?: string }) {
  return (
    <div className="rounded-lg border border-foreground/[0.08] bg-foreground/[0.03] overflow-hidden mb-4">
      {filename && (
...
function TipBlock({ text }: { text: string }) {
  return (
    <div className="rounded-lg border border-foreground/15 bg-foreground/[0.04] px-4 py-3 mb-4">
      <p className="text-sm text-foreground/70 leading-relaxed">
        <span className="font-mono font-semibold text-foreground/50 mr-2">TIP</span>
        {text}
      </p>
    </div>
  )
...
function WarningBlock({ text }: { text: string }) {
  return (
    <div className="rounded-lg border border-foreground/15 bg-foreground/[0.04] px-4 py-3 mb-4">
      <p className="text-sm text-foreground/70 leading-relaxed">
        <span className="font-mono font-semibold text-foreground/50 mr-2">IMPORTANT</span>
        {text}
      </p>
    </div>
  )
