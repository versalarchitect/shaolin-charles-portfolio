...
const content: LessonContent = {
  ...
  steps: [
    ...
    {
      type: 'code-fill',
      instruction: 'Complétez cette configuration d\'architecture événementielle. Remplissez les noms d\'événements, les chemins de handlers et les politiques de retry.',
      language: 'yaml',
      template: 'event-bus:\n  events:\n    - name: ___\n      handler: ___\n      retry:\n        maxAttempts: ___\n        backoff: exponential\n\n    - name: ___\n      handler: ___\n      retry:\n        maxAttempts: 5\n        backoff: exponential\n\n    - name: ___\n      handler: src/features/notifications/on-order-shipped.ts\n      retry:\n        maxAttempts: ___\n        backoff: linear',
      blanks: [
        { id: 'event-1-name', answer: 'order.placed', alternatives: ['OrderPlaced', 'order_placed'], hint: 'L\'événement émis quand une nouvelle commande est créée', placeholder: 'nom d\'événement' },
        { id: 'event-1-handler', answer: 'src/features/payments/on-order-placed.ts', alternatives: ['src/features/payments/handlers/order-placed.ts', 'payments/on-order-placed.ts'], hint: 'Le module Paiements gère cet événement', placeholder: 'chemin du handler' },
        { id: 'event-1-retry', answer: '3', alternatives: ['5', '2'], hint: 'Le traitement des paiements devrait réessayer mais pas trop de fois', placeholder: 'nombre' },
        { id: 'event-2-name', answer: 'payment.completed', alternatives: ['PaymentCompleted', 'payment_completed', 'payment.succeeded'], hint: 'L\'événement émis quand le paiement réussit', placeholder: 'nom d\'événement' },
        { id: 'event-2-handler', answer: 'src/features/orders/on-payment-completed.ts', alternatives: ['src/features/orders/handlers/payment-completed.ts', 'orders/on-payment-completed.ts'], hint: 'Le module Commandes doit savoir que le paiement a réussi', placeholder: 'chemin du handler' },
        { id: 'event-3-name', answer: 'order.shipped', alternatives: ['OrderShipped', 'order_shipped', 'shipment.created'], hint: 'L\'événement émis quand une commande est expédiée', placeholder: 'nom d\'événement' },
        { id: 'event-3-retry', answer: '2', alternatives: ['3', '1'], hint: 'Les notifications sont moins critiques que les paiements', placeholder: 'nombre' },
      ],
      filename: 'event-bus.config.yaml',
      explanation: 'Chaque événement connecte les modules sans imports directs. Les chemins de handlers pointent vers des fichiers à l\'intérieur du module CONSOMMATEUR — le module émetteur ne référence jamais le handler. Les politiques de retry varient selon la criticité : les paiements obtiennent des retries modérés (les échecs de paiement nécessitent un traitement attentif), tandis que les notifications en obtiennent moins (un courriel manqué est récupérable). Cette configuration est le contrat qui permet aux agents de construire chaque module indépendamment.',
    },
    {
      type: 'match',
      instruction: 'Associez chaque patron de découplage au mécanisme qu\'il utilise :',
      leftItems: [
        'Bus d\'événements',
      ...
      rightItems: [
        ...
        'Frontières type-safe',
        'Déploiement progressif',
        'Rétrocompatibilité',
      ],
      correctPairs: { 0: 0, 1: 1, 2: 2, 3: 3 },
      explanation: 'Chaque patron de découplage fonctionne à travers un mécanisme différent. Le bus d\'événements découple les modules via la communication asynchrone — aucun module n\'appelle un autre directement. Les contrats d\'interface créent des frontières type-safe contre lesquelles les agents peuvent implémenter indépendamment. Les feature flags permettent le déploiement progressif pour que le nouveau code puisse être livré sans affecter tous les utilisateurs. Le versionnage d\'API fournit la rétrocompatibilité pour que les consommateurs puissent migrer selon leur propre calendrier.',
    },
    {
      type: 'code-diff',
      title: 'Du couplage serré à l\'événementiel',
      body: 'Voyez comment une fonction fortement couplée qui importe directement depuis d\'autres modules est refactorisée pour utiliser un bus d\'événements, éliminant les dépendances inter-modules.',
      language: 'typescript',
      filename: 'src/features/orders/orders.service.ts',
      before: "// AVANT : Fortement couplé — Commandes importe depuis 3 autres modules\nimport { chargeCustomer } from '../payments/payments.service'\nimport { sendOrderConfirmation } from '../emails/email.service'\nimport { decrementStock } from '../inventory/inventory.service'\n\nexport async function placeOrder(input: PlaceOrderInput) {\n  const order = await db.orders.create(input)\n\n  // Les appels directs créent du couplage — tout changement à ces\n  // fonctions casse Commandes\n  await chargeCustomer(order.customerId, order.totalCents)\n  await sendOrderConfirmation(order.customerId, order.id)\n  await decrementStock(order.items)\n\n  return order\n}",
      after: "// APRÈS : Événementiel — Commandes ne connaît rien des autres modules\nimport { eventBus } from '@/infrastructure/event-bus'\n\nexport async function placeOrder(input: PlaceOrderInput) {\n  const order = await db.orders.create(input)\n\n  // Émet un événement — zéro connaissance de qui écoute\n  // Paiements, Courriel, Inventaire s'abonnent chacun indépendamment\n  eventBus.emit('order.placed', {\n    orderId: order.id,\n    customerId: input.customerId,\n    items: input.items,\n    totalCents: order.totalCents,\n  })\n\n  return order\n}",
      question: 'Combien d\'imports inter-modules ont été éliminés par ce refactoring ?',
      highlightLines: [2, 3, 4],
      explanation: 'Trois imports directs ont été éliminés : paiements, courriels et inventaire. Le module Commandes n\'a plus besoin de savoir que ces modules existent. Un agent construisant Commandes n\'a besoin que de comprendre le schéma d\'événement. Les agents construisant Paiements, Courriel ou Inventaire n\'ont qu\'à s\'abonner à l\'événement. Les quatre agents peuvent travailler simultanément avec zéro coordination.',
    },
    {
      type: 'checkpoint',
      xp: 2,
