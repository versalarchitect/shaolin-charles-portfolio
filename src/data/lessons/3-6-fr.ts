...
const content: LessonContent = {
  ...
  steps: [
    ...
    { type: 'terminal', instruction: "Vérifie ce que l'agent d'authentification a réellement produit (lister les fichiers) :", expectedCommand: 'find src/auth -type f -name "*.ts" | sort', hint: 'Utilise find pour lister tous les fichiers TypeScript dans le répertoire auth', platforms: { windows: { expectedCommand: 'Get-ChildItem -Recurse -Path "src/auth" -Filter "*.ts" -File | Sort-Object Name' } } },
